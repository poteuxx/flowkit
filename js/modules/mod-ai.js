...
export default class AIModule {
        ...
        this.container.innerHTML = `
            <div class="ai-console glass-card" style="max-width: 900px; margin: 0 auto; height: 100%; display: flex; flex-direction: column; background: rgba(2,6,23,0.8);">
                <div class="console-header" style="border-bottom: 1px solid var(--border-glass); padding-bottom: 1rem; margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center;">
                    <span style="font-weight: 800; color: var(--accent-primary); letter-spacing: 2px;">POTEUXX AI STATION</span>
                    <span class="status-badge" style="background: rgba(34, 211, 238, 0.1); color: var(--accent-primary); padding: 0.25rem 0.75rem; border-radius: 2rem; font-size: 0.7rem;">READY</span>
                </div>
                
                <div id="ai-chat" style="flex-grow: 1; overflow-y: auto; padding-right: 1rem; display: flex; flex-direction: column; gap: 1.5rem; font-family: 'Fira Code', monospace;">
                    <div class="ai-msg">
                        <p style="font-size: 0.8rem; color: var(--accent-primary); font-weight: 800; margin-bottom: 0.5rem;">SYSTEM</p>
                        <p style="color: var(--text-primary);">Assistant Poteuxx initialisé. Comment puis-je vous aider aujourd'hui ?</p>
                    </div>
                </div>
    ...
    setupHandlers() {
        ...
        const pushMsg = (role, text) => {
            ...
            msg.innerHTML = `
                <p style="font-size: 0.8rem; color: ${role === 'USER' ? 'var(--text-secondary)' : 'var(--accent-primary)'}; font-weight: 800; margin-bottom: 0.5rem; letter-spacing: 1px;">${role}</p>
                <p style="color: var(--text-primary); line-height: 1.5;">${text}</p>
            `;
